...
void Tache4::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	//Subscribers
	dureeTache =noeud.subscribe("/Cmde_tache4/lancementTache4", 10, &Tache4::get_dureeTacheCallback, this);
	...
	posteDeposee = noeud.subscribe("/Cmde_P4/ProduitPose",10,&Tache4::PieceDeposeeCallback,this); ///****** <---- MODIFICATION

	postePris = noeud.subscribe("/Cmde_P4/ProduitPrise",10,&Tache4::PiecePriseCallback,this); ///****** <---- MODIFICATION

	pubFINI= noeud.advertise<std_msgs::Int32>("/commande/Simulation/ProduitTraitement4fini",10);
...
{
	if((numTable == 4)&&(Prise==1)&&(position==4)/*&&(bras == 1)&&(pince == 1)*/) ///****** <---- MODIFICATION
	{
...
void Tache4::PiecePriseCallback(const std_msgs::Int32::ConstPtr& msg) ///****** <---- MODIFICATION
{
	Prise=1;
}

void Tache4::PieceDeposeeCallback(const std_msgs::Int32::ConstPtr& msg) ///****** <---- MODIFICATION
{
	Deposee=1;
}
...
void Tache4::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	...
	while(finTraitement != -1)
	{
		...
		{
			if((position==4)&&(Deposee==1)/*&&(bras==0)&&(pince==0)*/)
			{
				...
				if (iteratorPMap != ProductsMap.end() && (duree>=0)) // Vrai si l'itérateur n'est pas hors de la liste
				{
					...
					time = t0;
					while((time - t0 < duree))//productPointer->jobTime)
					{
						client_simGetVrepTime.call(srv_simGetVrepTime);
...
void Tache4::get_dureeTacheCallback(const std_msgs::Int32::ConstPtr& msg)
{
	duree=msg->data;
}
