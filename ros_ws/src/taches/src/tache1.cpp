...
void Tache1::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	//Subscribers
	dureeTache =noeud.subscribe("/Cmde_tache1/lancementTache1", 10, &Tache1::get_dureeTacheCallback, this);
	...
	postePris = noeud.subscribe("/Cmde_P1/ProduitPrise",10,&Tache1::PiecePriseCallback,this); ///****** <---- MODIFICATION

	pubFINI= noeud.advertise<std_msgs::Int32>("/commande/Simulation/ProduitTraitement1fini",10);
...
void Tache1::FinTraitementProduit(int numTable)
{
	...
	if((numTable == 1)&&(Prise==1)&&(position==1)/*&&(bras == 1)&&(pince == 1)*/) ///****** <---- MODIFICATION
	{
...
void Tache1::PiecePriseCallback(const std_msgs::Int32::ConstPtr& msg) ///****** <---- MODIFICATION
{
	Prise=1;
}
...
void Tache1::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	...
	while(finTraitement != -1)
	{
		...
		{
			if((position==1)&&(Deposee==1)/*&&(bras==0)&&(pince==0)*/) ///****** <---- MODIFICATION
			{
...
void Tache1::get_dureeTacheCallback(const std_msgs::Int32::ConstPtr& msg)
{
	duree=msg->data;
}
