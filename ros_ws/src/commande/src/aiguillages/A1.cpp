...
A1::A1(ros::NodeHandle nh)
{
	...
	getHandle = nh.subscribe("/HandleExchange/SendHandleShToA1", 1000, &A1::getHandleCallback, this);
	

	SendHandleShToA2 = nh.advertise<std_msgs::Int32>("/HandleExchange/SendHandleShToA2", 1000);

	SendHandleShToPoste3 = nh.advertise<std_msgs::Int32>("/HandleExchange/SendHandleShToPoste3", 1000);
...
void A1::getHandleCallback(const std_msgs::Int32::ConstPtr& msg)
{
	...
	if (PS==0 && PS_mem==1)
	{
		if (listeHandle.size()!=0)
		{
			HandleSh_msg.data=listeHandle[0];
	  		if (Aig_D==1)
			{
				cout << "Envoie handle à Poste3 de A1 " << HandleSh_msg.data<< endl;
				SendHandleShToPoste3.publish(HandleSh_msg);
				listeHandle.erase(listeHandle.begin());
			}
		
			if (Aig_G==1)
			{
				cout << "Envoie handle A2 de A1 " << HandleSh_msg.data << endl;
...
int A1::get_PS1()  //<---MODIF
{
	return PS;
}

int A1::get_posAig_Droit()
{
	return Aig_D;
}


int A1::get_posAig_Virage()
{
	return Aig_G;
}
