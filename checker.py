...
# Produits : 1 = 14 / 2 = 24 / 3 = 34 / 4 = 44 / 5 = 54 / 6 = 64
# --> ex: P = 24, le '2' indique le n° du produit, et le '4' indique que c'est un produits
...
test = 1 # Cette variable indique s'il y a une erreur dans le log: 1 - PAS D'ERREUR  /  0 - ERREUR
erreur_config = 0 # Dit si il y a une erreur de déclaration dans config par l'étudiant: 1 - ERREUR / 0 - PAS ERREUR
...
for ligne in contenu : # on travail ligne par ligne, donc produit par produit
    ...
    if nom != "\n":
        ...
        # NOM PRODUIT
        produit_num = int(nom)
        if produit_num < 1 or produit_num > 6:
            erreur_config = 1
        ...
        tache_list = conf[1]
        tache_var = []
        tache_list = list(tache_list)
        taille = len(tache_list)
        j = 0 # compte
        ...
        for i in range(taille_dest):
            if tache_var[i] < 1 or tache_var[i] > 8:
                erreur_config = 1
...
if erreur_config == 0:
    for line in Log :
        ...
        if ID == "NewProduct":
            ...
            if P_type != 4 and nb_produit[P-1] != 0:
                test = 0
            ...
            if P != 0: # Donc si la sortie n'est pas vide, qu'il y a bien un produit qui est evacuer
                ...
                if nb_produit[P-1] != 0 and nb_produit_log[P-1] <= nb_produit[P-1]: # cette condition sert à afficher les erreurs seulement sur les produits désiré. Si des produit sont créer mais non désiré il y a un autre message d'erreur qui se génère (en bas du code), donc pas besoin de faire plusieurs messages d'erreur sur des produits non désiré, juste 2 suffit (pour dire que produit non désiré a été crée, et que produit non désiré a été sortie)
                    if produit_final != production[P-1]:
                        test = 0
                        print('ERREUR: enchainement taches de la version {} du produit {} doit être {} et pas {}'.format(nb_produit_log[P-1],P,production[P-1][1:4],produit_final[1:4]))
                    if P_type != 4:
                        test = 0
                        print("ERREUR: la base de la version {} du produit {} n'est pas défini comme un produit".format(nb_produit_log[P-1],P))
                    if D1_type == 2:
                        test = 0
                        print("ERREUR: la tache {} de la version {} du produit {} n'est pas terminé lors de la sortie du produit".format(D1,nb_produit_log[P-1],P))
                    if D2_type == 2:
                        test = 0
                        print("ERREUR: la tache {} de la version {} du produit {} n'est pas terminé lors de la sortie du produit".format(D2,nb_produit_log[P-1],P))
                    if D3_type == 2:
                        test = 0
                        print("ERREUR: la tache {} de la version {} du produit {} n'est pas terminé lors de la sortie du produit".format(D3,nb_produit_log[P-1],P))
                    if D1_type != 2 and D1_type != 3:
                        test = 0
                        print("ERREUR: la tache {} de la version {} du produit {} n'est pas défini comme une tache".format(D1,nb_produit_log[P-1],P))
                    if D2_type != 2 and D2_type != 3:
                        test = 0
                        print("ERREUR: la tache {} de la version {} du produit {} n'est pas défini comme une tache".format(D2,nb_produit_log[P-1],P))
                    if D3_type != 2 and D3_type != 3:
                        test = 0
                        print("ERREUR: la tache {} de la version {} du produit {} n'est pas défini comme une tache".format(D3,nb_produit_log[P-1],P))
    ...
    if test == 1:
        for i in range(6):
            if nb_produit[i] != 0:
                ...
                if production[i][3] == 0:
                    if production[i][2] == 0:
                        print("Le produit {} a été assemblé {} fois avec une seule tâche: {}".format(i+1, nb_produit_log[i], production[i][1]))
                    else:
                        print("Le produit {} a été assemblé {} fois avec l'empilement de 2 tâches: {}".format(i+1, nb_produit_log[i], production[i][1:3]))
                else:
                    print("Le produit {} a été assemblé {} fois avec l'empilement de 3 tâches: {}".format(i+1, nb_produit_log[i], production[i][1:4]))
