...
Poste4::Poste4(ros::NodeHandle nh,  std::string executionPath)
{
	...
	pub_updateTache= nh.advertise<std_msgs::Int32>("/P4_Tache/ProduitDone", 10);

	getHandle = nh.subscribe("/HandleExchange/SendHandleShToPoste4", 1000, &Poste4::getHandleCallback, this);

	SendHandleShtoA2 = nh.advertise<std_msgs::Int32>("/HandleExchange/SendHandleShToA2", 1000);

	robotPosition = nh.subscribe("/robot/PositionRobot2", 10, &Poste4::PositionRobotCallback, this);
...
// Pour recevoir et envoyer l'etat des navettes	
	...
	ProductFromPost = nh.advertise<std_msgs::Int32>("/Cmde_P4/ProduitPrise", 20);
...
void Poste4::PositionRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	position = msg->data;
}

void Poste4::getHandleCallback(const std_msgs::Int32::ConstPtr& msg)
{
...
void Poste4::StopSensorCallback(const std_msgs::Int32::ConstPtr& msg)
{
	...
	if (PS3==0 && PS_mem==1)
	{
		...
		if (listeHandle.size()!=0)
		{
			...
			SendHandleShtoA2.publish(HandleSh_msg);
...
void Poste4::ProductPutOnShuttleCallback(const std_msgs::Int32::ConstPtr& msg){
	if (position==4)
	{
		cout << "condition depose sur table ok  " << msg_traite.data << endl;
