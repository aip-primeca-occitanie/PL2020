...
//Envoie du handle au poste 2

SendHandleShToPoste2 = nh.advertise<std_msgs::Int32>("/HandleExchange/SendHandleShtoPoste2", 20);


//Récupération de la position du robot

robotPosition = nh.subscribe("/robot/PositionRobot1", 10, &Poste1::PositionRobotCallback, this);
...
// Pour arrêter et démarrer les navettes
	...
	ProductFromPost = nh.advertise<std_msgs::Int32>("/Cmde_P1/ProduitPrise", 20);
...
// Pour gérer les couleurs des plateformes
	...
	getHandleShFromA11 = nh.subscribe("/HandleExchange/SendHandleShtoPoste1", 1000, &Poste1::getHandleShFromA11Callback, this);


//POur dire à la commande que la tache est finie

	pub_updateTache= nh.advertise<std_msgs::Int32>("/P1_Tache/ProduitDone", 10);
...
void Poste1::getHandleShFromA11Callback(const std_msgs::Int32::ConstPtr& msg)
{
...
void Poste1::StopSensorCallback(const std_msgs::Int32::ConstPtr& msg)
{
	...
	if (PS21==0 && PS_mem==1)
	{
		if (listeHandle.size()!=0)
		{
			...
			SendHandleShToPoste2.publish(HandleSh_msg);
...
void Poste1::PositionRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	position = msg->data;
}
